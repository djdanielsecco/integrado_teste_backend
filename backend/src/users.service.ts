...
export class UsersService {
    ...
    async signUp(user: Users) {
        ...
        try {
            let response = await this.userModel.insertMany(newUser);
    ...
    async verifyLogin(email: string, password: string) {
        try {
            const user: UsersDocument = await this.userModel.findOne({ email: `${email}` })
            const Email: string = user?.email
            const Password: string = user?.password
            const Name: string = user?.name
            const ID: string = user?._id.valueOf() as Object as string
    ...
    async changePassword(email: string, password: string, new_password: string) {
        try {
            ...
            if (verify) {
                ...
                let doc = await this.userModel.findOneAndUpdate({ email: email }, { password: keyCrypt },{new:true})
                console.log('doc: ', doc);
               if(!!doc) {return { message: "New password updated" }} else {
